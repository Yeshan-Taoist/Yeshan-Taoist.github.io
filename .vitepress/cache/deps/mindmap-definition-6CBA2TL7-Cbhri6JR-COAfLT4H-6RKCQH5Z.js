...
// node_modules/vitepress-plugin-legend/dist/mindmap-definition-6CBA2TL7-Cbhri6JR-COAfLT4H.js
var it = { exports: {} };
var rt = { exports: {} };
var nt = { exports: {} };
var Ot2 = nt.exports;
...
var ot = (function() {
  var O = q(function(D, g, L, n) {
    ...
    performAction: q(function(D, g, L, n, u, l, v) {
      var A = l.length - 1;
      switch (u) {
        case 6:
        case 7:
...
` + this.showPosition(), {
      ...
      performAction: q(function(g, L, n, u) {
        switch (n) {
          case 0:
  ...
  return q(N, "Parser"), N.prototype = m, m.Parser = N, new N();
})();
ot.parser = ot;
var Gt = ot;
var bt = {
...
var Ft = (tt = class {
  ...
  addNode(O, M, p, T) {
    Ot.info("addNode", O, M, p, T);
    const f = An();
    let i = f.mindmap?.padding ?? wU.mindmap.padding;
    switch (T) {
  ...
  getType(O, M) {
    switch (Ot.debug("In get type", O, M), O) {
      case "[":
  ...
  decorateNode(O) {
    if (!O)
      return;
    const M = An(), p = this.nodes[this.nodes.length - 1];
    O.icon && (p.icon = yc(O.icon, M)), O.class && (p.class = yc(O.class, M));
  }
  type2Str(O) {
    switch (O) {
...
q(dt, "insertPolygonShape");
var $t = q(function(O, M, p) {
...
var Wt = q(async function(O, M, p, T, f) {
...
`);
  ...
  const y = a.node().getBBox(), [o] = w2(f.fontSize);
...
function pt(O, M) {
  M.edges().map((p, T) => {
    ...
    if (p[0]._private.bodyBounds) {
      ...
      Ot.trace("Edge: ", T, f), O.insert("path").attr(
        "d",
...
function ft(O, M) {
  return new Promise((p) => {
    const T = bn("body").append("div").attr("id", "cy").attr("style", "display:none"), f = Jn({
...
function yt(O, M) {
  M.nodes().map((p, T) => {
    ...
    Ot.info("id:", T, "Position: (", p.position().x, ", ", p.position().y, ")", f), i.attr(
      "transform",
...
q(yt, "positionNodes");
var Vt = q(async (O, M, p, T) => {
  Ot.debug(`Rendering mindmap diagram
` + O);
  ...
  pt(r, a), yt(f, a), Rvt(
    void 0,
    e,
    t.mindmap?.padding ?? wU.mindmap.padding,
    t.mindmap?.useMaxWidth ?? wU.mindmap.useMaxWidth
  );
}, "draw");
var zt = {
  draw: Vt
...
var qt = q((O) => {
  ...
  for (let p = 0; p < O.THEME_COLOR_LIMIT; p++)
    O["lineColor" + p] = O["lineColor" + p] || O["cScaleInv" + p], _f(O["lineColor" + p]) ? O["lineColor" + p] = pe(O["lineColor" + p], 20) : O["lineColor" + p] = Se(O["lineColor" + p], 20);
  for (let p = 0; p < O.THEME_COLOR_LIMIT; p++) {
    const T = "" + (17 - 3 * p);
    M += `
...
//# sourceMappingURL=mindmap-definition-6CBA2TL7-Cbhri6JR-COAfLT4H-6RKCQH5Z.js.map
